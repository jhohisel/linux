...
static void pmic_glink_pdr_callback(int state, char *svc_path, void *priv)
{
	...
	list_for_each_entry(client, &pg->owners, node)
		client->pdr_notify(client->priv, state);
}
...
static void pmic_glink_rpmsg_remove(struct rpmsg_device *rpdev)
{
	...
	/* XXX: what if the platform_deivce went away */
...
static int pmic_glink_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pg->pdr))
		dev_err_probe(&pdev->dev, PTR_ERR(pg->pdr), "failed to initalize pdr\n");

	service = pdr_add_lookup(pg->pdr, "tms/servreg", "msm/adsp/charger_pd");
	if (IS_ERR(service))
		return dev_err_probe(&pdev->dev, PTR_ERR(service),
