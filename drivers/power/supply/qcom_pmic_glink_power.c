...
static void pmic_glink_power_pdr_notify(void *priv, int state)
{
	struct pmic_glink_power *pgp = priv;
	int ret;

	if (state == SERVREG_SERVICE_STATE_UP) {
		ret = pmic_glink_power_enable_notifications(pgp);
		if (ret)
			dev_err(pgp->dev, "failed to request power notifications\n");
	}
}
